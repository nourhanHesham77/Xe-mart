...
<!-- Mirrored from www.thetahmid.com/themes/xemart-v1.0/06-shopping-cart.php by HTTrack Website Copier/3.x [XR&CO'2014], Mon, 17 Jan 2022 22:15:46 GMT -->
<head>
        <meta charset="utf-8">
                    ...
                    <div class="col-lg-4 col-md-2">
                        <div class="carts-area d-flex">
                            ...
                            <div class="cart-box ml-4">
                                <a href="#" data-toggle="tooltip" data-placement="top" title="Shopping Cart" class="cart-btn">
                                    <img src="images/cart.png" alt="cart">
                                    <span class="count-cart" style="color:white;"><?php
                                    $id=$_SESSION['login'];
          $row_state=$connect->query("SELECT count(*) AS total FROM cart WHERE session_login='$id'");
          $values=mysqli_fetch_assoc($row_state);
...
$select_product= $connect->query("SELECT * FROM products WHERE id='$product'");
    ...
    foreach($select_product as $row_product){
        $img=explode(",",$row_product['img']);
                                    ...
                                    ?>
                                    <tr class="delete_mem<?php echo $row_cart['id'] ?>">
                                        <td class="t-pro d-flex">
                                            <div class="t-img">
                                                <a href="#"><img style="width:50px;height:70px;" src="admin/imges/<?php echo$img[0];?>" alt=""></a>
                                            </div>
                                            <div class="t-content">
                                                <p class="t-heading"><a href="#"><?php echo$row_product['name']?></a></p>
                                        ...
                                        <td class="_total_price"><?php echo$row_product['price'] ?></td>
                                        <td class="t-qty">
                                            <div class="qty-box">
                                                <div class="quantity buttons_added">
                                                    <input type="button" id="dec-val" mn-val="<?php echo$row_cart['id'] ?>" value="-" class="minus">
                <input type="number" id="orign" step="1" min="1" max="10" value="<?php echo$row_cart['count'];?>" class="qty text nmp" size="4" readonly>
                                                    <input type="button" id="inc-val" mn-val="<?php echo$row_cart['id'] ?>" value="+" class="plus">
                                                </div>
                                            </div>
                                        </td>
                                        <td class="_tot_val"><?php
                                         $origin=$row_product['price'];
                                         $num=$row_cart['count'];
                                         $total=$num * $origin;
                                         echo$total;
                                         ?></td>
                                        <td class="t-rem"><a style="cursor:pointer;" class="remo" carid="<?php echo$row_cart['id']?>"><i class="fa fa-trash-o"></i></a></td>
                                    </tr>
                        ...
                        <div class="shipping">
                            ...
                            <form action="#">
                                <div class="country-box">
                                    <select class="country" id="cty">
                                        <option hidden value="none" id="alrt">Select Country</option>
                                        <?php
                                        $select_country=$connect->query("SELECT * FROM tax");
                                ...
                                <div class="post-box">
                                    ...
                                    <button type="button" class="gt-est">Get Estimate</button>
                                </div>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="col-md-4">
                        <div class="crt-sumry">
                            <h5>Cart Summery</h5>
                            <ul class="list-unstyled">
                            <li class="putall">Subtotal <span class="sub-totl">
                            <?php
                                $id=$_SESSION['login'];
                                $get_cart=$connect->query("SELECT * FROM cart WHERE session_login='$id'");
                                $grand = 0;
                                ...
                                <li>Shipping & Tax <span class="tax">$</span></li>
                                <li>Grand Total <span class="grand-totl">$00.00</span></li>
                            </ul>
                            <div class="cart-btns text-right">
                                <button type="button" class="up-cart">Update Cart</button>
                                <button type="button" class="chq-out chout"><a href="functions/add_report.php" >Checkout</a></button>
                              <div class="jsigned"></div>
                            </div>
                        </div>
                    </div>
                </div>
          ...
          $(document).ready(function(){
            ...
            $(".remo").click(function(){
              ...
              var par= $(this).closest("tr");
                $(".delete_mem"+id).fadeOut();
                $.post("functions/delete_cart.php",{
                      xid : id
                }
                ,function(bdata){
                    console.log(bdata); 
                    $(".putall").load(location.href+' .putall');     
                })
             })


             $(".plus").click(function(){
 var mid =  $(this).attr("mn-val");
 var coun = $(".nmp").val();
 var pric= $(this).closest("tr").find("._total_price").html();
 var totl= $(this).closest("tr").find("._tot_val"); 

var couns;
 
 $.post("functions/increament_cart.php",{
    lid : mid,
    scoun:coun
 }
 ,function(data){
 ...
 var fid =  $(this).attr("mn-val");
 var coun = $(".nmp").val();
 var pric= $(this).closest("tr").find("._total_price").html();
 var totl= $(this).closest("tr").find("._tot_val"); 
var couns;

 $.post("functions/decreament_cart.php",{
    sid : fid,
    scoun:coun
 }
 ,function(mdata){
    couns = mdata;
       ...
       $(".putall").load(location.href+' .putall');
             ...
             $('.gt-est').click(function(e){
                e.preventDefault();
                var cty =$('#cty').val();
                var sub =$(".sub-totl").html();
                if(cty=="none"){
$('#alrt').html("please select country").addClass("alert");
                }
                
                if(cty!="none"){
                    $.post("functions/get_tax.php",{
                        tid : cty
                    },function(tdata){
                        $(".tax").html(tdata);
                        $(".grand-totl").html(Number(sub)+Number(tdata));
                    })
                }
...
e.preventDefault();
                ...
                if(sub=="$"&&cty!="none"){
                    $('.jsigned').css('color','red').html("please get estimated first");
                    e.preventDefault();
                }
             })
